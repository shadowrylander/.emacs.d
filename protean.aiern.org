...
#+begin_src emacs-lisp
(defmacro meq/wrapped-call (buffer-name &rest args)
    (message buffer-name)
    (apply #'meq/call (meq/rs (pop args)) (format "*%s*" buffer-name) (map #'meq/rs args))
    (message "%s...done" buffer-name))
#+end_src
...
(cond (meq/var/phone (let ((pm* "pkg")
                        ...
                        (eval `(meq/wrapped-call "Adding unstable repo" ,pm ,install unstable-repo))
                        (eval `(meq/wrapped-call "Adding root repo" ,pm ,install root-repo))
                        (meq/wrapped-call "Updating pkgs" pkg update -y)
                        (mapc #'(lambda (pkg) (interactive)
                            (meq/install-package pkg pm)) '(;; clang
                                                                ;; fftw
                                                                ;; freetype
                                                                gitea
                                                                ;; libcrypt
